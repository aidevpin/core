...
# ultronai_hash
RUN git clone https://github.com/raptor3um/ultronai_hash
RUN cd ultronai_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} ultronai
RUN useradd -u ${USER_ID} -g ultronai -s /bin/bash -m -d /ultronai ultronai
...
RUN mkdir /ultronai-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /ultronai-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /ultronai-src
