...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/UltronaiParams"
else
    PARAMS_DIR="$HOME/.ultronai-params"
fi
...
DOWNLOAD_URL="https://github.com/ultronaiorg/core/releases/tag/1.3.17.03"
...
# fetch methods can be disabled with UTO_DISABLE_SOMETHING=1
UTO_DISABLE_WGET="${UTO_DISABLE_WGET:-}"
#UTO_DISABLE_IPFS="${UTO_DISABLE_IPFS:-}"
UTO_DISABLE_CURL="${UTO_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$UTO_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$UTO_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$UTO_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the Ultronai parameters!
...
This script will fetch the Ultronai SNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Ultronai SNARK parameters. Note that it is
