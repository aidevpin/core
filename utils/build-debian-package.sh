...
PACKAGE_VERSION=$($SRC_PATH/src/ultronaid --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/ultronaid $DEB_BIN
cp $SRC_PATH/src/ultronai-cli $DEB_BIN
cp $SRC_PATH/util/fetch-params.sh $DEB_BIN/ultronai-fetch-params
# Copy docs
...
# Copy manpages
cp $SRC_DOC/man/ultronaid.1 $DEB_MAN
cp $SRC_DOC/man/ultronai-cli.1 $DEB_MAN
cp $SRC_DOC/man/ultronai-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/ultronaid.bash-completion $DEB_CMP/ultronaid
cp $SRC_PATH/contrib/ultronai-cli.bash-completion $DEB_CMP/ultronai-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/ultronaid.1
gzip --best -n $DEB_MAN/ultronai-cli.1
gzip --best -n $DEB_MAN/ultronai-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/ultronaid $DEB_BIN/ultronai-cli
