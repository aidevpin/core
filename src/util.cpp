...
const char * const BITCOIN_CONF_FILENAME = "ultronai.conf";
const char * const BITCOIN_PID_FILENAME = "ultronaid.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "ultronaid -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\UltronaiCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\UltronaiCore
    // Mac: ~/Library/Application Support/UltronaiCore
    // Unix: ~/.ultronaicore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "UltronaiCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/UltronaiCore";
#else
    // Unix
    return pathRet / ".ultronaicore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty ultronai.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    // Check for untranslated substitution to make sure Ultronai Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Ultronai Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2021, nEndYear) + "The Ultronai Core developers";
    }
