...
void static UltronaiMiner(const CChainParams& chainparams)
{
    LogPrintf("UltronaiMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ultronai-miner");
    ...
    CWallet * pWallet = NULL;
        ...
        if (!EnsureWalletIsAvailable(pWallet, false)) {
            LogPrintf("UltronaiMiner -- Wallet not available\n");
        }
    #endif
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("UltronaiMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            ...
            LogPrintf("UltronaiMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("UltronaiMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("UltronaiMiner -- runtime error: %s\n", e.what());
        return;
    }
}

int GenerateUltronais(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++){
        minerThreads->create_thread(boost::bind(&UltronaiMiner, boost::cref(chainparams)));
    }
    return(numCores);
}
